...
async function addBackButton(response,url){
  ...
  const widget=`<div id="woodrickGlobalNav" style="position:fixed;left:14px;top:58px;z-index:2147483646;display:flex;gap:8px;align-items:center"><button id="woodrickGlobalBack" type="button" aria-label="Go back one step" style="width:44px;height:44px;border-radius:999px;border:2px solid #f0c96b;background:#080808;color:#f0c96b;font:900 22px/1 Arial;cursor:pointer;box-shadow:0 6px 18px rgba(0,0,0,.28);display:grid;place-items:center">←</button><a id="woodrickGlobalHome" href="/" aria-label="Back to home" title="Back to Home" style="height:44px;padding:0 12px;border-radius:999px;border:1px solid #f0c96b;background:#080808;color:#f0c96b;text-decoration:none;font:900 11px/44px Arial;box-shadow:0 6px 18px rgba(0,0,0,.22)">HOME</a></div><script id="woodrick-back-nav-v2">(function(){var b=document.getElementById('woodrickGlobalBack');if(!b)return;b.addEventListener('click',function(){var picker=document.getElementById('wwExactPicker');if(picker&&picker.classList.contains('open')){picker.classList.remove('open');var st=document.getElementById('wwStatus');if(st)st.textContent='Back to Woodrick Library. Your selected Mood Board designs are kept.';return}var anyModal=document.querySelector('.ww-picker-modal.open,[role="dialog"].open,.modal.open');if(anyModal){anyModal.classList.remove('open');return}if(history.length>1){history.back();return}if(location.pathname==='/voice-design-assistant.html'||location.pathname==='/voice-design-assistant'){location.href='/design-your-space.html';return}location.href='/';});})();</script>`;
  const alignScript=`<script id="woodrick-global-nav-align-v1">(function(){function align(){var nav=document.getElementById('woodrickGlobalNav'),logo=document.querySelector('.nav .logo,header .brand,header .logo,.topbar .brand,header a:first-child');if(!nav||!logo)return;var r=logo.getBoundingClientRect();if(r.bottom<=0)return;nav.style.top=Math.max(8,Math.round(r.top+(r.height-44)/2))+'px'}function ready(){requestAnimationFrame(align);setTimeout(align,120);setTimeout(align,600)}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',ready,{once:true});else ready();addEventListener('resize',align);addEventListener('orientationchange',function(){setTimeout(align,150)})})();</script>`;
