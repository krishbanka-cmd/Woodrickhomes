...
async function addBackButton(response,url){
  ...
  const widget=`<style id="woodrick-global-nav-position-v3">#woodrickGlobalNav{position:fixed;left:auto;right:max(14px,calc((100vw - 1180px)/2));top:16px;z-index:2147483646;display:flex;gap:8px;align-items:center}body.woodrick-library-page #woodrickGlobalNav{top:76px}@media(max-width:620px){#woodrickGlobalNav{right:10px;top:10px;gap:6px}body.woodrick-library-page #woodrickGlobalNav{top:68px}#woodrickGlobalBack{width:40px!important;height:40px!important}#woodrickGlobalHome{height:40px!important;padding:0 10px!important;line-height:40px!important}}</style><div id="woodrickGlobalNav"><button id="woodrickGlobalBack" type="button" aria-label="Go back one step" style="width:44px;height:44px;border-radius:999px;border:2px solid #f0c96b;background:#080808;color:#f0c96b;font:900 22px/1 Arial;cursor:pointer;box-shadow:0 6px 18px rgba(0,0,0,.28);display:grid;place-items:center">←</button><a id="woodrickGlobalHome" href="/" aria-label="Back to home" title="Back to Home" style="height:44px;padding:0 12px;border-radius:999px;border:1px solid #f0c96b;background:#080808;color:#f0c96b;text-decoration:none;font:900 11px/44px Arial;box-shadow:0 6px 18px rgba(0,0,0,.22)">HOME</a></div><script id="woodrick-back-nav-v3">(function(){if(location.pathname.indexOf('/woodrick-library')===0)document.body.classList.add('woodrick-library-page');var b=document.getElementById('woodrickGlobalBack');if(!b)return;b.addEventListener('click',function(){var picker=document.getElementById('wwExactPicker');if(picker&&picker.classList.contains('open')){picker.classList.remove('open');var st=document.getElementById('wwStatus');if(st)st.textContent='Back to Woodrick Library. Your selected Mood Board designs are kept.';return}var anyModal=document.querySelector('.ww-picker-modal.open,[role="dialog"].open,.modal.open');if(anyModal){anyModal.classList.remove('open');return}if(history.length>1){history.back();return}if(location.pathname==='/voice-design-assistant.html'||location.pathname==='/voice-design-assistant'){location.href='/design-your-space.html';return}location.href='/';});})();</script>`;
  const alignScript='';
