...
async function customCategories(env){
  ...
  return r.objects.map(o=>clean((o.customMetadata||{}).categoryName)).filter(Boolean).sort((a,b)=>a.localeCompare(b));
}
...
const adminPatch=`<style id="woodrick-category-manager-style-v1">
...
<script id="woodrick-category-manager-v1">(function(){
var STATIC=['Plywood','Laminates','Louvers','Acrylic Laminates','Shuttering Plywood','Doors','HDHMR & MDF','WPC Board & Chaukhat','Cement','Tiles','Sanitaryware','Bath Fittings','uPVC Doors & Windows','Hardware','Furniture & Kitchen Hardware','Flooring','Wallpapers','Decorative Panels','More Products'];
...
export default{async fetch(request,env,ctx){
  ...
  if(isAdmin&&!html.includes('woodrick-category-manager-v1'))html=html.replace('</body>',adminPatch+'\n</body>');
  if(isProducts&&!html.includes('woodrick-public-dynamic-categories-v1'))html=html.replace('</body>',productPatch+'\n</body>');
  const h=new Headers(response.headers);h.delete('content-length');h.set('cache-control','no-store, no-cache, must-revalidate, max-age=0');h.set('x-woodrick-version','dynamic-category-manager-v1');return new Response(html,{status:response.status,statusText:response.statusText,headers:h});
}};
