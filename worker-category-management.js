...
async function refreshCategories(){var custom=await getCustom(),all=unique(STATIC.concat(custom));setSelect('pCategory',all);setSelect('lCategory',all);setSelect('woodrickEditCategory',all);window.WOODRICK_ALL_CATEGORIES=all.slice();var chips=document.getElementById('categoryChips');if(chips)chips.innerHTML=all.map(function(x){return'<span class="category-chip '+(custom.some(function(c){return c.toLowerCase()===x.toLowerCase()})?'custom':'')+'">'+esc(x)+'</span>'}).join('');return all}
...
export default{async fetch(request,env,ctx){
  ...
  const h=new Headers(response.headers);h.delete('content-length');h.set('cache-control','no-store, no-cache, must-revalidate, max-age=0');h.set('x-woodrick-version','dynamic-category-manager-v2-edit-sync');return new Response(html,{status:response.status,statusText:response.statusText,headers:h});
}};
