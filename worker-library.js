...
async function imageViewer(key,obj,env){
  ...
  const catalogueRoot=key.includes('/jpg/')?key.split('/jpg/')[0]:'';
  const pdfKey=catalogueRoot?`${catalogueRoot}/original/${slug(meta.catalogue||meta.title||'catalogue')}.pdf`:'';
  const pdfExists=pdfKey?await env.PRODUCT_MEDIA.head(pdfKey):null;
  const downloadButton=pdfExists?`<a class="download" href="/api/media?download=1&key=${encodeURIComponent(pdfKey)}">DOWNLOAD CATALOGUE</a>`:'';
  ...
  const body=`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${htmlEsc(title)} | Woodrick Homes</title><style>*{box-sizing:border-box}body{margin:0;background:#090909;color:#fff;font-family:Arial,sans-serif}.top{position:sticky;top:0;z-index:5;background:#050505;border-bottom:2px solid #f0c96b;padding:12px 16px;display:flex;gap:10px;align-items:center;justify-content:space-between;flex-wrap:wrap}.back,.nav,.download{display:inline-flex;align-items:center;justify-content:center;padding:11px 15px;border:1px solid #f0c96b;color:#f0c96b;text-decoration:none;font-weight:900;font-size:12px}.download{background:#15120c}.nav.next{background:#f0c96b;color:#111}.disabled{opacity:.35;pointer-events:none}.title{font-family:Georgia,serif;font-size:18px;text-align:center;flex:1}.stage{min-height:calc(100vh - 72px);display:flex;align-items:center;justify-content:center;padding:18px}.stage img{max-width:100%;max-height:calc(100vh - 110px);object-fit:contain;background:#fff}.counter{font-size:12px;color:#ccc;white-space:nowrap}@media(max-width:620px){.top{justify-content:center}.title{order:-1;flex-basis:100%}.stage{padding:8px}.stage img{max-height:calc(100vh - 150px)}}</style></head><body><div class="top"><a class="back" href="/products/#media">← BACK</a>${navButton('← PREVIOUS',prev)}<div class="title">${htmlEsc(title)} <span class="counter">Page ${current} of ${pages.length}</span></div>${downloadButton}${navButton('NEXT →',next,'next')}</div><div class="stage"><img src="${raw}" alt="${htmlEsc(title)} page ${current}"></div></body></html>`;
