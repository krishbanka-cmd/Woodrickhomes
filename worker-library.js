...
function bearerAuthorized(request,env){
  ...
  return !!env.ADMIN_UPLOAD_TOKEN && auth===`Bearer ${env.ADMIN_UPLOAD_TOKEN}`;
}
  ...
  async fetch(request,env,ctx){
    ...
    if(url.pathname==='/api/upload'&&request.method==='POST'){
      ...
      if(contentType.includes('multipart/form-data')||contentType.includes('application/x-www-form-urlencoded')){
        let form;
        try{form=await request.clone().formData()}catch{return base.fetch(request,env,ctx);}
        if(String(form.get('action')||'')==='delete-library'){
          if(!bearerAuthorized(request,env)) return json({error:'Invalid admin access code'},401);
          let keys=[];
          const raw=String(form.get('keys')||'').trim();
          if(raw){
            try{const parsed=JSON.parse(raw);if(Array.isArray(parsed))keys=parsed;}catch{return json({error:'Invalid delete keys'},400);}
          }
