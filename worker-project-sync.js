...
function patchVoiceWorkspace(html){
  ...
  if(!html.includes('woodrick-library-search-v1')){
    const searchUi=`<style id="woodrick-library-search-v1">.ww-library-search{width:100%;margin:0 0 10px;padding:12px 13px;border:1px solid #cdbb9d;border-radius:9px;background:#fff;font:600 14px/1.3 Arial,sans-serif}.ww-library-search:focus{outline:2px solid #d2a64e;outline-offset:1px}</style><script id="woodrick-library-search-v1">window.addEventListener('DOMContentLoaded',function(){function install(){var cards=document.getElementById('wwCards'),filters=document.getElementById('wwFilters');if(!cards||!filters||document.getElementById('wwLibrarySearch'))return false;var input=document.createElement('input');input.id='wwLibrarySearch';input.className='ww-library-search';input.type='search';input.placeholder='Search brand, category, catalogue, page or Design No.';input.setAttribute('aria-label','Search verified Woodrick designs');filters.parentNode.insertBefore(input,filters);var busy=false;function apply(){if(busy)return;busy=true;requestAnimationFrame(function(){var q=String(input.value||'').trim().toLowerCase(),shown=0,total=0;cards.querySelectorAll('.ww-card').forEach(function(card){var match=!q||String(card.innerText||'').toLowerCase().indexOf(q)>=0,show=match&&shown<5;if(match)total++;card.style.display=show?'':'none';if(show)shown++});cards.querySelectorAll('.ww-thickness-group,.ww-brand-group').forEach(function(group){group.style.display=group.querySelector('.ww-card:not([style*="display: none"])')?'':'none'});var status=document.getElementById('wwLibraryStatus');if(status)status.textContent=total?(shown+' of '+total+' verified matches shown. Search the exact brand, page or Design No. to refine.'):'No verified match found. Try brand, category, catalogue, page or Design No.';busy=false})}input.addEventListener('input',apply);new MutationObserver(apply).observe(cards,{childList:true,subtree:true});apply();return true}if(!install()){var tries=0,t=setInterval(function(){tries++;if(install()||tries>30)clearInterval(t)},200)}});<\/script>`;
    html=html.replace('</head>',searchUi+'</head>');
  }
  return html;
}
