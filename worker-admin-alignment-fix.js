...
.live-grid{align-items:start!important}.hierarchy-back{grid-column:1/-1!important;justify-self:start!important}.media-preview img,.pdf-cover{object-fit:contain!important;object-position:center center!important}.pdf-live-cover{width:100%!important;height:100%!important;border:0!important;background:#fff!important;display:block!important}
</style>
<script id="woodrick-product-hierarchy-script-v2">(function(){
var hierarchyParams=new URLSearchParams(location.search),selectedBrand=hierarchyParams.get('brand')||'',selectedCategory=hierarchyParams.get('category')||'';if(selectedCategory)activeCategory=selectedCategory;
...
function automaticCover(x){var q=new URLSearchParams({source:String(x.sourceKey||''),brand:brandOf(x),category:String(x.category||''),catalogue:catalogueOf(x)});return '/api/catalogue-cover?'+q.toString()}
function coverOf(x){var src=String(x.coverUrl||coverFallbacks[x.key]||automaticCover(x));return src.indexOf('/catalogue-covers/')===0?src.split('?')[0]+'?v=20260914-1':src}
function setBrandUrl(brand,mode){var u=new URL(location.href);if(activeCategory&&activeCategory!=='all')u.searchParams.set('category',activeCategory);else u.searchParams.delete('category');if(brand)u.searchParams.set('brand',brand);else u.searchParams.delete('brand');history[mode||'replaceState']({woodrickHierarchy:brand?'catalogues':'brands',brand:brand||''},'',u.pathname+u.search+u.hash)}
function ensureHierarchyHistory(){var current=new URL(location.href);if(!selectedBrand){history.replaceState({woodrickHierarchy:'brands',brand:''},'',current.pathname+current.search+current.hash);return}if(history.state&&history.state.woodrickHierarchy==='catalogues')return;var detail=current.pathname+current.search+current.hash,base=new URL(current);base.searchParams.delete('brand');history.replaceState({woodrickHierarchy:'brands',brand:''},'',base.pathname+base.search+base.hash);history.pushState({woodrickHierarchy:'catalogues',brand:selectedBrand},'',detail)}
function catalogueHref(x){var href=mediaUrl(x),join=href.indexOf('?')>=0?'&':'?';return href+join+'title='+encodeURIComponent(catalogueOf(x))+'&return='+encodeURIComponent(location.pathname+location.search+'#media')}
...
function card(g){
  ...
  if(image)preview='<img loading="lazy" decoding="async" src="'+mediaUrl(image)+'" alt="'+esc(g.catalogue)+'"><div class="cover-title">'+esc(g.catalogue)+'</div>';
  else if(pdf){var exactPdfCover=String(pdf.key||'')==='kitchen/pdf/woodrick-kitchen-catalogue.pdf';preview=(exactPdfCover?'<iframe class="pdf-live-cover" loading="lazy" title="'+esc(g.catalogue)+' full cover" src="'+esc(mediaUrl(pdf))+'&raw=1#page=1&toolbar=0&navpanes=0&scrollbar=0&view=Fit"></iframe>':cover?'<img class="pdf-cover" loading="lazy" decoding="async" src="'+esc(cover)+'" data-cover-fallback="'+esc(automaticCover(pdf))+'" data-pdf="'+esc(mediaUrl(pdf))+'" alt="'+esc(g.catalogue)+' cover">':'<div class="pdf-cover-fallback">PDF CATALOGUE</div>')+'<div class="cover-title">'+esc(g.catalogue)+'</div>'}
  else if(video)preview='<video preload="metadata" muted src="'+mediaUrl(video)+'"></video><div class="cover-title">'+esc(g.catalogue)+'</div>';
  var acts='';if(pdf)acts+='<a class="open-media" href="'+catalogueHref(pdf)+'" target="_self">OPEN PDF</a>';if(image)acts+='<a class="open-media" href="'+mediaUrl(image)+'" target="_blank" rel="noopener">OPEN PHOTO</a>';if(video)acts+='<a class="open-media" href="'+mediaUrl(video)+'" target="_blank" rel="noopener">OPEN VIDEO</a>';
...
renderMedia=function(){
  ...
  if(!items.length){
    ...
    liveStatus.textContent='Loading latest catalogues…';mediaGrid.innerHTML='<div class="empty-media">Preparing Woodrick product catalogues…</div>';return
  }
  if(activeCategory==='all'){mediaGrid.innerHTML=groups.map(card).join('');return}
  if(!selectedBrand){var brands={};items.forEach(function(x){var b=brandOf(x);(brands[b]||(brands[b]=[])).push(x)});mediaGrid.innerHTML=Object.keys(brands).sort().map(function(b){var bg=grouped(brands[b]);var first=bg[0],thumb='';if(first){var im=first.items.find(function(x){return mediaType(x)==='image'}),pd=first.items.find(function(x){return mediaType(x)==='pdf'}),pc=pd?coverOf(pd):'';if(im)thumb='<img loading="lazy" decoding="async" src="'+mediaUrl(im)+'" alt="'+esc(b)+'">';else if(pd&&pc)thumb='<img class="pdf-cover" loading="lazy" decoding="async" src="'+esc(pc)+'" data-cover-fallback="'+esc(automaticCover(pd))+'" data-pdf="'+esc(mediaUrl(pd))+'" alt="'+esc(b)+'">';else if(pd)thumb='<div class="pdf-cover-fallback">PDF</div>'}return '<article class="media-card brand-choice" data-brand="'+esc(b)+'"><div class="media-preview">'+(thumb||esc(b))+'<div class="cover-title">'+esc(b)+'</div></div><div class="media-info"><div class="media-category">'+esc(activeCategory)+'</div><div class="media-title">'+esc(b)+'</div><div class="catalogue-media-label">'+bg.length+' catalogue'+(bg.length===1?'':'s')+'</div><div class="media-actions" style="margin-top:12px"><button class="open-media" type="button">VIEW CATALOGUES</button></div></div></article>'}).join('');return}
