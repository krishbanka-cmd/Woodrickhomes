...
function parseJsonField(v){try{const x=JSON.parse(String(v||''));return x&&typeof x==='object'?x:null}catch(_){return null}}
function materialName(x){if(!x||typeof x!=='object')return'';return [clean(x.category,80),clean(x.brand,80),clean(x.catalogue,120),x.sku?`SKU ${clean(x.sku,80)}`:(x.page?`Catalogue page ${clean(x.page,20)}`:'')].filter(Boolean).join(' · ')}
...
async function handleAiDesign(request,env){
  try{
    ...
    const change=clean(incoming.get('change'),1200)||'improve furniture, storage, lighting, finishes and decor as appropriate';
    const furniture=clean(incoming.get('furniture'),900)||'appropriate furniture for the room type';
    const reference=clean(incoming.get('reference'),600)||'Woodrick recommended design direction';
    const concept=clean(incoming.get('concept'),1200);
    const mapping=parseJsonField(incoming.get('materialMapping'))||{};
    const selected=parseJsonField(incoming.get('selectedMaterials'))||[];
    const mappedLines=Object.keys(mapping).map(surface=>{const name=materialName(mapping[surface]);return name?`${clean(surface,80)} MUST use ${name}`:''}).filter(Boolean);
    const selectedLines=Array.isArray(selected)?selected.map(materialName).filter(Boolean):[];
    const materialLock=mappedLines.length?`MANDATORY VERIFIED WOODRICK MATERIAL LOCK. These assignments are customer-approved and must not be substituted, moved to another surface, recoloured into a different finish, or replaced with an invented material: ${mappedLines.join('; ')}. Treat catalogue page references as exact verified visual/material references when no SKU is available. Never invent a SKU or design number. Other selected approved materials available for complementary use only where appropriate: ${selectedLines.join('; ')||'none'}.`:'';
    const required=[];const ft=(furniture+' '+change).toLowerCase();if(/double bed|\bbed\b|बेड/.test(ft))required.push('Double Bed');if(/wardrobe|almari|almirah|अलमारी/.test(ft))required.push('Wardrobe');if(/tv unit|tv panel|television/.test(ft))required.push('TV Unit');if(/dresser|dressing/.test(ft))required.push('Dresser');if(/headboard/.test(ft))required.push('Headboard');
    const furnitureLock=required.length?`MANDATORY FURNITURE: ${required.join(', ')}. Every listed item must be clearly present and recognizable in the generated room. If space is tight, use a compact practical version; never omit a mandatory item.`:'';
    const lockPrompt=mode==='Improve My Current Design'?`STRICT CURRENT-ROOM IMPROVEMENT MODE. This must remain unmistakably the same real room. Preserve the exact camera/viewpoint, wall positions, corners, ceiling and floor boundaries, doors, windows, columns, AC/fixed services, openings and all requested protected elements. Protected elements: ${preserve}. Only redesign or improve these requested elements: ${change}. Do not invent structural changes or move fixed openings.`:`DESIGNS BY WOODRICK MODE. Keep the actual room shell, camera/viewpoint, walls, ceiling/floor geometry, doors, windows, columns and fixed architectural openings true to the uploaded photo. Within that real shell, Woodrick may redesign furniture layout, storage, joinery, lighting and decor, but must obey all mandatory furniture and locked material assignments. Customer direction: ${change}. Preserve where requested: ${preserve}.`;
    const prompt=`Photorealistic premium interior visualization for Woodrick Homes. ${lockPrompt} Space: ${roomType}. Room dimensions: ${length||'?'} ft x ${width||'?'} ft${height?` x ${height} ft`:''}. Furniture/elements: ${furniture}. ${furnitureLock} Style/colours/materials: ${style}. Budget: ${budget}. Fixed points/openings: ${openings}. Woodrick design/SKU/catalogue reference: ${reference}. ${materialLock} ${concept?`Planning direction: ${concept}.`:''} Preserve the approved layout intent and practical circulation. Keep believable scale and realistic construction. All mandatory furniture and customer-locked Woodrick material placements have higher priority than decorative creativity. Do not add people, text, labels or logos.`;
    ...
    return json({ok:true,image:`data:image/jpeg;base64,${image}`,model:'gpt-image-1',mode,materialLockApplied:mappedLines.length>0,lockedSurfaces:Object.keys(mapping)});
...
.woodrick-actions-compact .hey-woodrick-float{width:52px!important;height:52px!important}.woodrick-actions-compact .hey-woodrick-mic{width:40px!important;height:40px!important}.woodrick-actions-compact .float-actions a{position:relative;width:52px!important;height:52px!important}.woodrick-actions-compact .float-actions .whatsapp:before{font-size:22px!important}.woodrick-actions-compact .float-actions .call:before{font-size:21px!important}.woodrick-actions-compact .hey-woodrick-float:after,.woodrick-actions-compact .float-actions a:after{position:absolute;bottom:50%;padding:8px 11px;border-radius:6px;background:#080808;color:#f0c96b;border:1px solid #d6aa4f;font:800 10px/1 Arial,sans-serif;white-space:nowrap;opacity:0;pointer-events:none;transform:translateY(50%) scale(.94);transition:opacity .18s ease,transform .18s ease}.woodrick-actions-compact .hey-woodrick-float:after{content:'HEY WOODRICK';left:58px}.woodrick-actions-compact .float-actions .whatsapp:after{content:'WHATSAPP ENQUIRY';right:58px}.woodrick-actions-compact .float-actions .call:after{content:'CALL NOW';right:58px}.woodrick-actions-compact .hey-woodrick-float:hover:after,.woodrick-actions-compact .hey-woodrick-float:focus-visible:after,.woodrick-actions-compact .float-actions a:hover:after,.woodrick-actions-compact .float-actions a:focus-visible:after{opacity:1;transform:translateY(50%) scale(1)}
.hey-woodrick-float{position:fixed;left:18px;bottom:22px;z-index:9999;display:flex;align-items:center;gap:10px;padding:10px 15px 10px 10px;border:1px solid #d6aa4f;border-radius:999px;background:#090909;color:#f0c96b!important;text-decoration:none!important;font:800 12px/1 Arial,sans-serif;letter-spacing:.02em;box-shadow:0 12px 34px rgba(0,0,0,.28);transition:all .2s ease}.hey-woodrick-float.nav-integrated{position:static;width:auto;height:44px;margin:0 0 0 2px;padding:4px 11px 4px 5px;gap:7px;box-shadow:none}.hey-woodrick-float.nav-integrated .hey-woodrick-mic{width:30px;height:30px;font-size:14px;box-shadow:none}.hey-woodrick-float.nav-integrated>span:last-child{display:block;font-size:10px}.hey-woodrick-float.nav-integrated small{display:block;font-size:8px}.hey-woodrick-float:hover{transform:translateY(-2px);box-shadow:0 16px 38px rgba(0,0,0,.34)}.hey-woodrick-mic{width:38px;height:38px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(145deg,#d5a338,#f0c96b);color:#111;font-size:19px;box-shadow:0 0 0 5px rgba(240,201,107,.10)}.hey-woodrick-float small{display:block;margin-top:3px;color:#d9ccb0;font-size:9px;font-weight:600;letter-spacing:0}.woodrick-actions-compact .hey-woodrick-float{left:10px;bottom:18px;width:48px;height:48px;padding:5px;justify-content:center}.woodrick-actions-compact .hey-woodrick-float>span:last-child{display:none}.woodrick-actions-compact .hey-woodrick-mic{width:36px;height:36px;box-shadow:none}.woodrick-actions-compact .float-actions{right:10px;bottom:18px;gap:7px}.woodrick-actions-compact .float-actions a{width:48px;height:48px;padding:0;font-size:0;display:grid;place-items:center}.woodrick-actions-compact .float-actions .whatsapp:before{content:'💬';font-size:20px}.woodrick-actions-compact .float-actions .call:before{content:'📞';font-size:19px}.voice-arrival{margin:0 0 18px;padding:12px 14px;border-left:3px solid #c59a58;background:#fff8e8;color:#5c4931;font:700 12px/1.5 Arial,sans-serif;border-radius:4px}@media(max-width:520px){.hey-woodrick-float{left:10px;right:auto;bottom:18px;padding:8px 11px 8px 8px}.hey-woodrick-mic{width:34px;height:34px;font-size:17px}.hey-woodrick-float small{display:none}}
</style>
<a class="hey-woodrick-float" href="/voice-design-assistant.html?new=1" aria-label="Start a new Hey Woodrick design" title="Hey Woodrick — Start New Design"><span class="hey-woodrick-mic">🎙</span><span>HEY WOODRICK<small>Start New Design</small></span></a><script>(function(){var w=document.querySelector('.hey-woodrick-float'),n=document.getElementById('woodrickGlobalNav');if(w&&n){w.classList.add('nav-integrated');var a=n.querySelector('.woodrick-global-actions');n.insertBefore(w,a||null);return}if(location.pathname==='/'||location.pathname==='/index.html'){var compact=function(){document.body.classList.toggle('woodrick-actions-compact',scrollY>520)};compact();addEventListener('scroll',compact,{passive:true})}})();<\/script>`;

const DESIGN_PREFILL=`<script id="hey-woodrick-prefill">(function(){if(!location.pathname.endsWith('/design-your-space.html'))return;var p=new URLSearchParams(location.search);if(p.get('voice')!=='1')return;function set(name,key){var el=document.querySelector('[name="'+name+'"]');var v=p.get(key||name);if(el&&v)el.value=v;}set('roomType');set('length');set('width');set('height');set('openings');set('style');set('change');var ref=p.get('reference');if(ref){var ch=document.querySelector('[name="change"]');if(ch&&!ch.value.includes(ref))ch.value=(ch.value?'\\n':'')+'Preferred Design No. / SKU: '+ref;}var form=document.getElementById('spaceForm');if(form){var note=document.createElement('div');note.className='voice-arrival';note.textContent='🎙 Hey Woodrick has filled the details it understood. Please verify dimensions, Design/SKU and openings, then add your room photo.';form.insertBefore(note,form.firstChild);form.scrollIntoView({behavior:'smooth',block:'start'});}})();<\/script>`;
