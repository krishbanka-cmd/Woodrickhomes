...
.hey-woodrick-float{position:fixed;right:18px;bottom:92px;z-index:9999;display:flex;align-items:center;gap:10px;padding:10px 15px 10px 10px;border:1px solid #d6aa4f;border-radius:999px;background:#090909;color:#f0c96b!important;text-decoration:none!important;font:800 12px/1 Arial,sans-serif;letter-spacing:.02em;box-shadow:0 12px 34px rgba(0,0,0,.28);transition:transform .2s ease,box-shadow .2s ease}.hey-woodrick-float:hover{transform:translateY(-2px);box-shadow:0 16px 38px rgba(0,0,0,.34)}.hey-woodrick-mic{width:38px;height:38px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(145deg,#d5a338,#f0c96b);color:#111;font-size:19px;box-shadow:0 0 0 5px rgba(240,201,107,.10)}.hey-woodrick-float small{display:block;margin-top:3px;color:#d9ccb0;font-size:9px;font-weight:600;letter-spacing:0}.voice-arrival{margin:0 0 18px;padding:12px 14px;border-left:3px solid #c59a58;background:#fff8e8;color:#5c4931;font:700 12px/1.5 Arial,sans-serif;border-radius:4px}@media(max-width:520px){.hey-woodrick-float{right:10px;bottom:72px;padding:8px 11px 8px 8px}.hey-woodrick-mic{width:34px;height:34px;font-size:17px}.hey-woodrick-float small{display:none}}
</style>
...
const DESIGN_PREFILL=`
<script id="hey-woodrick-prefill">
(function(){
  if(!location.pathname.endsWith('/design-your-space.html')) return;
  var p=new URLSearchParams(location.search); if(p.get('voice')!=='1') return;
  function set(name,key){var el=document.querySelector('[name="'+name+'"]');var v=p.get(key||name);if(el&&v)el.value=v;}
  set('roomType');set('length');set('width');set('height');set('openings');set('style');set('change');
  var ref=p.get('reference');if(ref){var ch=document.querySelector('[name="change"]');if(ch&&!ch.value.includes(ref))ch.value=(ch.value?ch.value+'\n':'')+'Preferred Design No. / SKU: '+ref;}
  var form=document.getElementById('spaceForm');if(form){var note=document.createElement('div');note.className='voice-arrival';note.textContent='🎙 Hey Woodrick has filled the details it understood. Please verify dimensions, Design/SKU and openings, then add your room photo.';form.insertBefore(note,form.firstChild);form.scrollIntoView({behavior:'smooth',block:'start'});}
})();
</script>`;

async function withVoiceUi(response,url){
  const type=response.headers.get('content-type')||'';
  if(!response.ok||!type.includes('text/html')) return response;
  let html=await response.text();
  if(url.pathname!=='/voice-design-assistant.html'&&!html.includes('hey-woodrick-global-style')) html=html.replace(/<\/body>/i,VOICE_WIDGET+'\n</body>');
  if(url.pathname.endsWith('/design-your-space.html')&&!html.includes('hey-woodrick-prefill')) html=html.replace(/<\/body>/i,DESIGN_PREFILL+'\n</body>');
...
export default{
  async fetch(request,env,ctx){
    ...
    return withVoiceUi(response,url);
  }
};